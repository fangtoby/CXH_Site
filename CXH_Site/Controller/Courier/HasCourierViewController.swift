//
//  HasCourierViewController.swift
//  CXH_Site
//
...
import MJRefresh
/// 已揽件
class HasCourierViewController:BaseViewController{
    ...
    override func viewDidLoad() {
        ...
        table.mj_header=MJRefreshNormalHeader(refreshingBlock:{ () -> Void in
            self.pageNumber=1
            if self.identity == 2{
                self.queryGiveHistory(self.pageNumber, pageSize:10, isRefresh:true, statu:4)
            }else{
                self.queryGiveHistoryForDriver(self.pageNumber, pageSize:10, isRefresh:true, statu:5)
            }
        })
        table.mj_footer=MJRefreshAutoNormalFooter(refreshingBlock: { () -> Void in
            self.pageNumber+=1
            if self.identity == 2{
                self.queryGiveHistory(self.pageNumber, pageSize:10, isRefresh:false, statu:4)
            }else{
...
// MARK: - 网络请求
extension HasCourierViewController{
...
extension HasCourierViewController:UITableViewDelegate,UITableViewDataSource,CouierTableViewCellDelegate{
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if arr.count > 0{
            ...
            cell!.delegate=self
            cell!.updateCell(entity,statu:4)
        }
        return cell!
    }
...
// MARK: - 空视图协议
extension HasCourierViewController:DZNEmptyDataSetDelegate,DZNEmptyDataSetSource{
    //图片
